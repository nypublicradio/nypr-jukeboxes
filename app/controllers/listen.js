...
export default Controller.extend({
  ...
  _tracksAreStale: function() {
    ...
    } else if (this.get('nowPlaying').hasCurrentTrack) {
      trackStartTimeTs = this.get('nowPlaying.trackStartTimeTs'); // only the current track is from the current show
    }

    return (trackStartTimeTs < showStartTimeTs) || (trackStartTimeTs > showStartTimeTs + this.SHOW_STALE_CUTOFF);
  },

  // playlistHistoryItems: computed('model.recentTracks', function() {
    // let currentAiring = this.model.playlist.dailySchedule.airings.findBy('isCurrent', true)
    // return (currentAiring.tracks || []).slice(0, 3);
    //
    //
    //
    // debugger
    ...
    //   let currentTrackStartTime = moment(this.nowPlaying.trackStartTimeTs * 1000).format("hh:mm A");
